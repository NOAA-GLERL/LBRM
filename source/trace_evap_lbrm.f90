   ...
   CONTAINS
      ...
      ELSE                                              ! calibration run, so compute Cons
         ...
         !  Next line added for Priestley-Taylor variant B. Lofgren 09/21 
            IF (LBRM_PET_METHOD .eq. 3) NRad = LData%Met%Values(4, MetIndx)    ! face net radiation in W/m2
            write(*,*) NRad
      ...
      DO Seq = LData%Parm%SDateSeq, LData%Parm%EDateSeq
         ...
         Precip = LData%Met%Values(3, MetIndx) / 1000.0 * LData%Parm%Area  ! mm converted to cubic meters
         !  Next line added for Priestley-Taylor variant B. Lofgren 09/21
         NRad    = LData%Met%Values(4, MetIndx)                             ! Surface net radiation in W/m2
         !
...
!--------------------------------------------------------------------------------      
     ...
     &                (P-D) * EPN(-D*CID) + N/(P-B) * EPN(-B*CID) + O/P
      ...
      R = R + US - USR * (1. - EPN(-P * CID)) / P   ! Why is this defined twice?
      Ns = R * AlpSf / P
      Ss = T
...
!--------------------------------------------------------------------------
      ...
      IF (LBRM_PET_Method .gt. 1) THEN ! JAK; during cfg read, we enforced = 1,2, or 3
         ...
         DO D = 1, 31
            ...
            END IF
         END DO
         IF(ANY(Parm%AirTemps(:,:) .eq. MissingData_Real)) THEN
            write(*,*) 'some/all daily air temp climatology missing from parameter file'
            CALL EXIT(1)
         ! JAK copied the above loops to read in Parm%NetRads 
         ! this is NESTED under PET==2, bc you never RADS w/out TEMPS
         ELSEIF (LBRM_PET_Method .eq. 3) THEN      ! use PT method
            ...
            DO D = 1, 31
               ...
               END IF
            END DO
            IF(ANY(Parm%NetRads(:,:) .eq. MissingData_Real)) THEN
               write(*,*) 'some/all daily net radiation climatology missing from parameter file'
               CALL EXIT(1)
            END IF  
         END IF
      END IF
